...
import { decodeData } from '../utilities/encryption';
...
const PdfDecoder = () => {
    ...
    const decode = () => {
        if (text === "") {
            ...
            let result = decodeData(text, key);
            if (result.status) {
                setText(result.data);
            } else {
                alert(result.error)
    ...
    return (
        <>
            <HeaderComponent title="Pdf Decoder" />
                ...
                <div className='w-full h-auto flex flex-col justify-start items-center'>
                    <div className=' w-full grid grid-cols-12 gap-4 '>
                        ...
                        <div className='w-full col-span-4 pl-96 flex justify-between items-center md:pl-4 md:flex-col md:gap-4 md:col-span-12 md:order-3'>
                            <button className='bg-dark w-[100px] h-[40px] rounded text-light hover:bg-secondary' onClick={decode}>Decode</button>
